...
<AccordionGroup>
  ...
  <Accordion title="Driver modules are not loaded">
    ...
    If the driver is built with DKMS, review whether the build was successful.
    ...
    <Tip>
      The driver needs to be rebuilt after the kernel version is updated. DKMS performs this operation
      automatically, but if it fails you can reinstall the package.
    </Tip>
  </Accordion>
  ...
  <Accordion title="A version mismatch is reported while loading the model">
    ...
    runtime version. The compilation steps are explained in the
    [Model Compilation](/en/boards/o1/ai/deepx/models) section.
  </Accordion>
